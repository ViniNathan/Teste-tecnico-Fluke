...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      rulesLogger.info({ ruleId: rule.id, versionId: version.id }, 'Rule created');
      ...
      res.status(201).json({
        ...
        current_version: {
          ...
          action: (() => {
            try {
              return typeof version.action === 'string' ? JSON.parse(version.action) : version.action;
            } catch (e) {
    ...
    const rules = result.rows.map((row) => ({
      ...
      current_version: row.version_id
        ? {
            ...
            action: (() => {
              try {
                return typeof row.action === 'string' ? JSON.parse(row.action) : row.action;
              } catch (e) {
                return row.action;
              }
            })(),
  ...
  try {
    ...
    try {
      ...
      if (payload.condition !== undefined || payload.action !== undefined) {
        ...
        rulesLogger.info(
          { ruleId, oldVersion: current.version, newVersion: current.version + 1 },
